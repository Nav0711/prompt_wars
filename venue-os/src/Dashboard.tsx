...
// react-router-dom available for future navigation
    ...
    <div className="app-layout">
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-alert">
          ...
          {/* User Info & Logout */}
          <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid var(--border-subtle)' }}>
            <div style={{
              width: '32px', height: '32px', borderRadius: '50%', flexShrink: 0,
              background: userProfile?.picture ? `url(${userProfile.picture}) center/cover` : 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))',
              display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '0.8rem', fontWeight: 600
            }}>
              {!userProfile?.picture && (userProfile?.name?.[0] || 'D')}
            </div>
            <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
              <p style={{ margin: 0, fontSize: '0.85rem', color: '#fff', fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{userProfile?.name || 'Demo Operator'}</p>
              <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{userProfile?.email || 'operator@venueos.demo'}</p>
            </div>
            <button onClick={handleLogout} className="btn-icon" title="Logout" style={{ padding: '6px' }}>
              <LogOut size={16} />
            </button>
          </div>
        </div>
